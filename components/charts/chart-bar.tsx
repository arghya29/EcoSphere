...
export function ChartBar({ data, title }: { data: { name: string; emissionsKg: number }[]; title: string }) {
  if (data.length === 0) {
    ...
    <div>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} layout="vertical" margin={{ left: 24, right: 16 }}>
          ...
          <XAxis type="number" tickFormatter={(v) => formatKg(v)} stroke="hsl(var(--muted-foreground))" fontSize={11} />
          ...
          <Bar dataKey="emissionsKg" fill="hsl(var(--scope3))" radius={[0, 4, 4, 0]} name="Emissions" />
        </BarChart>
      </ResponsiveContainer>
